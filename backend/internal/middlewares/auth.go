...
//
// # ทำไมอ่าน DB ทุก request ไม่เชื่อ claim ในโทเคนอย่างเดียว
//
// JWT เซ็นแล้วเรียกคืนไม่ได้ ข้างในคือภาพ ณ วินาทีที่ล็อกอิน ไม่ใช่ ณ ตอนนี้ ถ้าอ่าน role จาก claim:
//
//   - ลดสิทธิ์ admin → user แล้วคนนั้นยังสั่ง endpoint ของแอดมินได้จนโทเคนหมดอายุ (นานถึง 30 วัน)
//   - ลบบัญชีทิ้งแล้วโทเคนของบัญชีที่ไม่มีอยู่ยังผ่านด่านนี้ ไปตายเอาข้างใน
//   - เลื่อนขึ้นเป็น admin แล้วเจ้าตัวต้อง logout/login ใหม่ถึงจะใช้ได้ ทั้งที่ DB เปลี่ยนแล้ว
//
// ราคาที่จ่ายคือ SELECT ด้วย primary key หนึ่งครั้งต่อ request บนตารางไม่กี่ร้อยแถว ถูกกว่าที่
// handler ส่วนใหญ่ทำอยู่แล้ว (เดิมหลาย handler ยิง First(&user, userID) ซ้ำเองอยู่ดี — ตอนนี้
// อ่านรอบเดียวแล้วแชร์ผ่าน context ดู currentNamespaceID ใน controller/helper.go)
//
// claim "role" ยังออกให้เหมือนเดิมเพื่อความเข้ากันได้ แต่ไม่ถูกใช้ตัดสินสิทธิ์อีกต่อไป
