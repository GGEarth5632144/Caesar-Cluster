...
ตอนสมัคร เช็ค 2 ชั้น: (1) มี student_id นี้ในตารางไหม (2) สถานภาพยังเป็นนักศึกษาอยู่ไหม — ไม่ผ่านชั้นไหนก็สมัครไม่ได้
...
|---|---|---|
| GET | `/health` | ping DB |
| POST | `/api/register` | ต้องอยู่ใน `eligible_students` และสถานภาพยังเป็นนักศึกษา ไม่งั้น 403 — **ไม่คืน JWT** แต่ส่งลิงก์ยืนยันไปทางอีเมล (ส่งเมลไม่ออก = ยกเลิกการสมัครทั้งรายการ ไม่มีบัญชีค้าง) |
...
5. 🟠 **หน้าเว็บฝั่งแอดมินยังเป็นกล่องเปล่าอยู่หลายหน้า** — `AdminDashboard` (ซึ่งเป็นหน้าแรกที่
   แอดมินเห็นทันทีหลังล็อกอิน), `Auditlog`, `IPCmanagement`, `Service`
...
6. 🟠 **status ไม่ sync กับของจริง** — DB เขียน `running` ตอน deploy สำเร็จครั้งเดียว ถ้า pod พังทีหลัง DB ยังบอก `running`
   ต้องมี reconcile loop — ป้ายสถานะบนการ์ด service ยังโกหกอยู่
