...
เมื่อ `APP_ENV=production` seed จะทำสองอย่างต่างจากตอน dev โดยอัตโนมัติ:
...
### 5. เปลี่ยนรหัสผ่าน admin หลัง login ครั้งแรก

ถึงจะตั้ง `SEED_ADMIN_PASSWORD` ไว้แล้ว รหัสนั้นก็ยังค้างอยู่ในไฟล์ `.env` บนเครื่อง
login เข้าไปเปลี่ยนเป็นรหัสที่ไม่ได้เก็บไว้ที่ไหนอีกทีก่อนให้คนอื่นมาใช้งาน
...
1. **หน้าเว็บฝั่งแอดมินยังเป็นกล่องเปล่าหลายหน้า** `AdminDashboard` (หน้าแรกที่แอดมินเห็นหลังล็อกอิน), `Alertadmin`, `Auditlog`, `IPCmanagement`, `Service` ยังไม่ได้ทำ แต่เมนูใน sidebar ชี้ไปหาแล้ว — แอดมินที่เข้ามาครั้งแรกจะเจอกล่องสีเทาว่างๆ ต้องทำก่อนส่งมอบให้คนอื่นใช้
