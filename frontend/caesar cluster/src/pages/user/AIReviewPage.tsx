  ...
  return (
    <div className="mt-3 rounded-xl bg-[#1a1714] border border-white/8 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/8">
        ...
        <div className="flex items-center gap-1.5 text-sm text-white/30 font-mono">
      ...
      <div className="px-4 py-3 flex flex-col gap-1 min-h-[60px]">
        {doneLines.map((line, i) => (
          <p key={i} className="text-base font-mono text-green-400/60 leading-relaxed">
        ...
        {!allDone && partialText !== "" && (
          <p className="text-base font-mono text-green-300/90 leading-relaxed">
        ...
        {allDone && (
          <p className="text-base font-mono text-white/20 animate-pulse">
    ...
    <div className={cn(
      ...
      <div className="flex items-start gap-3">
        ...
        <div className={cn(
          ...
          {isRunning ? <Loader2 size={20} className="animate-spin" /> :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="shrink-0">
              {isRunning && (
                <span className="flex items-center gap-1 text-base font-mono text-[#F08B51]">
                  <Clock size={13} /> {elapsedTimer}
                </span>
              )}
              {isDone && (
                <span className="flex items-center gap-1 text-base font-bold text-green-700">
                  {latency ? (
                    <><Clock size={13} /> {(latency / 1000).toFixed(2)}s</>
                  ) : "Done"}
                </span>
              )}
              {isError && (
              ...
              {isPending && (
                <span className="text-base text-[#211a14]/25">Pending</span>
              )}
            </div>
          </div>
          ...
          {isDone && stage.doneText && (
            <p className="mt-2 text-base font-mono text-[#211a14]/55 bg-black/[0.03] rounded-lg px-2.5 py-1.5 border border-black/5">
              {stage.doneText}
            </p>
          )}
...
function TelemetryPanel({ tel, attemptCount }: { tel: TelemetrySnapshot; attemptCount: number }) {
  ...
  return (
    <div className="rounded-2xl border border-black/5 bg-[#FFFDF6] overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 text-base font-bold text-[#211a14]/70">
          <Activity size={16} className="text-[#BB6653]" />
          Performance Metrics
        </span>
        {open ? <ChevronUp size={16} className="text-[#211a14]/30" /> : <ChevronDown size={16} className="text-[#211a14]/30" />}
      </button>
        ...
        <div className="px-5 pb-4 flex flex-col gap-3 border-t border-black/5">
          <div className="grid grid-cols-2 gap-3 pt-3">
            <div className="flex flex-col gap-1">
              <p className="text-sm text-[#211a14]/40 uppercase tracking-wider">Circuit Breaker</p>
              <span className={cn("text-sm font-bold px-2.5 py-1 rounded-full w-fit", circuitColor)}>
                {tel.circuit_status}
              </span>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-sm text-[#211a14]/40 uppercase tracking-wider">Attempts</p>
              <p className="text-sm font-bold text-[#211a14]">{attemptCount} / {tel.max_retries}</p>
            </div>
            <div className="flex flex-col gap-1">
              <p className="text-sm text-[#211a14]/40 uppercase tracking-wider flex items-center gap-1">
                <Zap size={12} /> Tokens
              </p>
              <p className="text-sm font-mono text-[#211a14]/70">
          ...
          {Object.keys(tel.latencies_ms).length > 0 && (
            <div className="flex flex-col gap-1.5">
              <p className="text-sm text-[#211a14]/40 uppercase tracking-wider flex items-center gap-1">
                <Clock size={12} /> Agent Latency
              </p>
              <div className="flex flex-wrap gap-2">
                {Object.entries(tel.latencies_ms).map(([agent, ms]) => (
                  <span key={agent} className="text-sm font-mono bg-[#211a14]/5 px-2.5 py-1 rounded-lg text-[#211a14]/60">
                    {agent}: {ms}ms
                  </span>
                ))}
              </div>
    ...
    <div className="flex flex-col gap-6 text-left font-mono animate-in fade-in duration-200 pb-10">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-4xl font-bold text-[#211a14]">AI Review</h1>
            {info && (
              <div className="flex flex-wrap items-center gap-2 mt-1.5 text-sm text-[#211a14]/50">
                <span className="flex items-center gap-1">
                  <Box size={13} className="text-[#BB6653]" /> {info.image}
                </span>
        ...
        {/* Elapsed timer */}
        <div className={cn(
          "flex items-center gap-2 px-4 py-2 rounded-xl border font-mono text-base font-bold transition-colors",
          timerRunning
            ? "bg-[#FFF8E8] border-[#F08B51]/30 text-[#F08B51]"
            : "bg-green-50 border-green-100 text-green-700",
        )}>
          <Clock size={16} className={timerRunning ? "animate-pulse" : ""} />
          {elapsedTimer}
        </div>
      </div>
      ...
      {!info && recovering && (
        <div className="flex items-center gap-2.5 p-4 rounded-2xl bg-black/[0.02] border border-black/5">
          <Loader2 size={16} className="animate-spin text-[#211a14]/40" />
          <p className="text-sm text-[#211a14]/50">Recovering service details…</p>
        </div>
      )}
      {!info && !recovering && (
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-orange-50 border border-orange-100">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-base font-bold text-orange-800">Missing service details</p>
          ...
          <button
            type="button"
            onClick={() => navigate(`/${PATHS.services}`)}
            className="shrink-0 rounded-lg px-3 py-1.5 text-sm font-bold text-orange-700 border border-orange-200 hover:bg-orange-100 transition-colors"
          >
            Back to Services
          </button>
        </div>
      ...
      {reviewResult && (
        <div className={cn(
          "flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold w-fit",
          ...
          {reviewResult.status === "reviewing" && <Loader2 size={14} className="animate-spin" />}
          {reviewResult.status === "needs_confirmation" && <CheckCircle2 size={14} />}
          {reviewResult.status === "escalated" && <AlertTriangle size={14} />}
          {reviewResult.status === "failed" && <XCircle size={14} />}
          {{
        ...
        <div className="flex flex-col gap-4 pt-2">
          ...
          {isClean && (
            <div className="flex items-start gap-3 p-5 rounded-2xl bg-green-50 border border-green-100">
              ...
              <div>
                <p className="text-base font-bold text-green-800">All Clear — No Issues Found</p>
                {reviewResult?.justification && (
                  <p className="text-sm text-green-700/80 mt-1.5 leading-relaxed">
                    {reviewResult.justification}
                  </p>
                )}
              </div>
          ...
          {hasSuggestions && (
            <>
              ...
              <div className="flex flex-col gap-2">
                <p className="text-sm font-bold uppercase tracking-wider text-[#BB6653] flex items-center gap-1.5">
                  <Cpu size={14} /> AI Recommendation
                </p>
                <div className="p-4 rounded-2xl bg-[#FFFDF6] border border-black/5 text-base text-[#211a14]/75 leading-relaxed">
                  {reviewResult?.justification}
                  {reviewResult?.feedback_insight && (
                    <>
                      <div className="h-px bg-black/8 my-3" />
                      <p className="text-sm text-[#211a14]/55">{reviewResult.feedback_insight}</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <p className="text-sm font-bold uppercase tracking-wider text-[#BB6653] flex items-center gap-1.5">
                  <Terminal size={14} /> Proposed Changes
                </p>
                <div className="rounded-2xl bg-[#1a1714] border border-black/20 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/8">
                    <span className="text-sm text-white/30 font-mono">proposed_fix</span>
                  ...
                  <pre className="px-5 py-4 text-sm text-green-300/90 font-mono overflow-x-auto max-h-48 leading-relaxed whitespace-pre-wrap break-all">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-bold uppercase tracking-wider text-[#BB6653] flex items-center gap-1.5">
                  <Box size={14} /> Container Image for K8s Deploy
                </label>
                <div className="flex items-center gap-2 rounded-xl border-2 border-[#F08B51]/30 bg-white px-4 py-3 focus-within:border-[#F08B51]">
                  <input
                    ...
                    className="w-full bg-transparent text-base text-[#211a14] outline-none font-mono"
                    placeholder="Edit image URL if AI suggests a patched version..."
                  />
                </div>
                <p className="text-sm text-[#211a14]/35">
          ...
          {isEscalated && (
            <div className="flex items-start gap-3 p-5 rounded-2xl bg-orange-50 border border-orange-100">
              ...
              <div>
                <p className="text-base font-bold text-orange-800">Circuit Breaker Triggered</p>
                ...
                {reviewResult?.feedback_insight && (
                  <p className="text-sm text-orange-600/70 mt-2 font-mono bg-orange-100/60 rounded-lg p-2">
                    {reviewResult.feedback_insight}
                  </p>
                )}
              </div>
          ...
          {isFailed && (
            <div className="flex items-start gap-3 p-5 rounded-2xl bg-red-50 border border-red-100">
              ...
              <div>
                <p className="text-base font-bold text-red-800">AI Pipeline Error</p>
                {reviewResult?.error_message && (
                  <p className="text-sm text-red-700/80 mt-1 font-mono break-all">
                    {reviewResult.error_message}
                  </p>
                )}
              </div>
          ...
          {/* Deploy error */}
          {deployError && (
            <div className="flex items-start gap-2 p-3.5 rounded-xl bg-red-50 border border-red-100 text-sm text-red-600">
              <AlertTriangle size={16} className="shrink-0 mt-0.5" /> {deployError}
            </div>
          )}
          ...
          {info && (
            <div className="flex items-center justify-between gap-3 pt-2">
              <button
                type="button"
                onClick={() => navigate(`/${PATHS.services}`)}
                className="rounded-xl px-5 py-3 text-base font-bold text-[#211a14]/60 transition-colors hover:bg-black/5"
              >
                Back to Services
              </button>
              ...
              <div className="flex items-center gap-2">
                {hasSuggestions && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-5 py-3 text-base font-bold text-[#211a14]/60 transition-colors hover:bg-black/5 disabled:opacity-50"
                  >
                    <ThumbsDown size={16} /> Deploy Original
                  </button>
                )}
                ...
                {(isClean || hasSuggestions) && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-[#BB6653] px-6 py-3 text-base font-bold text-white shadow-md transition-all hover:bg-[#F08B51] disabled:opacity-60"
                  >
                    {deploying ? <Loader2 size={16} className="animate-spin" /> : <ThumbsUp size={16} />}
